...
v-if="!mapEnabled"
            ...
            <p class="text-sm">
                Mit dem Klick auf den Button wird Google Maps geladen. Dabei können
                personenbezogene Daten (z. B. Ihre IP-Adresse) an Google übermittelt
                werden.
            </p>
            <button
class="px-4 py-2 rounded-md bg-primary text-on-primary font-semibold hover:opacity-90 transition"
                type="button" @click="enableMap">
                Karte laden
            </button>
            <p class="text-xs">
                Details in der
                <NuxtLink to="/privacy" class="underline text-primary">Datenschutzerklärung</NuxtLink>.
            </p>
        </div>

        <iframe
v-else :src="src" class="w-full h-full" style="border: 0" allowfullscreen loading="lazy"
            referrerpolicy="no-referrer-when-downgrade" @load="onMapLoad" />

        <div
v-if="mapEnabled && !mapLoaded"
            class="absolute inset-0 flex items-center justify-center bg-white/70 dark:bg-slate-900/60">
            <div class="h-10 w-10 border-4 border-slate-300 border-t-slate-700 rounded-full animate-spin"/>
        </div>
    </div>
</template>
